...
class SmartthingsPublisher(Node):
    ...
    def __init__(self, smartthings_response, update_period):
        ...
        self.publisher_pills_motion_sensor = self.create_publisher(Bool, 'smartthings_pills_motion_sensor', 10)
        ...
        self.publisher_bedroom_sensor_door = self.create_publisher(Bool, 'smartthings_bedroom_sensor_door', 10)
        self.publisher_living_sensor_door = self.create_publisher(Bool, 'smartthings_living_sensor_door', 10)
        self.publisher_bathroom_sensor_door = self.create_publisher(Bool, 'smartthings_bathroom_sensor_door', 10)
    ...
    def timer_callback(self):
        if self.smartthings_response.updated:
            ...
            #motion sensors
            msg = Bool()
            msg.data = not self.smartthings_response.pills_motion_sensor_active
...
class SmartthingsResponse:
    def __init__(self, update_period):
        ...
        self.pills_motion_sensor_active = None
            ...
            while True:
                ...
                for device in devices:
                    ...
                    if device.label == 'bedroom_sensor_door':  # bedroom door sensor check
                        print('bedroom_door 1')
                        await device.status.refresh()
                        self.bedroom_door_sensor_closed = device.status.values['contact'] == 'closed'

                    if device.label == 'livingroom_sensor_door':  # livingroom door sensor check
                        print('livigroom_door 2')
                        await device.status.refresh()
